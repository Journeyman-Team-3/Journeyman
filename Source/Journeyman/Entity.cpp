...
void AEntity::TakeDamage(int32 _dmg)
{
	if (CanBeHit)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(TakeDamageCooldown, [&]()
			{
				GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Reset Taking Damage"));
				CanBeHit = true;
			}, AttackCooldownTime, false);
	}
}
